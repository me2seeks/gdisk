...
// CreateImage 创建图片验证码
func (*Captcha) CreateImage() string {
...
// Reload 重载
func (*Captcha) Reload(captchaId string) bool {
	return captcha.Reload(captchaId)
}

// Verify 验证
func (*Captcha) Verify(captchaId, val string) bool {
	return captcha.VerifyString(captchaId, val)
}

// GetImageByte 获取图片二进制流
func (*Captcha) GetImageByte(captchaId string) []byte {
